...
public class NatureRevivePlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        if (!Util.isFolia()) {
            NatureReviveBukkitLogger.error("&a您的伺服器不支援 Folia API!");
            NatureReviveBukkitLogger.warning("&c本版本的 NatureRevive 是專為 PaperMC 推出的新軟體 Folia 設計, 因此需要使用 Folia 才能運行該插件!");
            NatureReviveBukkitLogger.warning("&c您有可能是下載了錯誤版本的 NatureRevive, 請回到巴哈姆特, 重新下載!");
            NatureReviveBukkitLogger.warning("&c倘若您想了解 Folia, 請至 PaperMC 的官網檢閱!");
            NatureReviveBukkitLogger.warning("&c您的伺服器版本不支援 NatureRevive (Folia Supported Version): " + getServer().getVersion());
        ...
        if (nmsWrapper == null) {
            ...
            NatureReviveBukkitLogger.warning("&c您的版本有可能不支援 NatureRevive (Folia Supported Version): " + getServer().getVersion());
        ...
        getServer().getGlobalRegionScheduler().runAtFixedRate(this, (scheduledTask) -> {
            if (queue.size() > 0 && isSuitableForChunkRegeneration()) {
                for (int i = 0; i < readonlyConfig.taskPerProcess && queue.hasNext(); i++) {
                    ...
                    getServer().getRegionScheduler().execute(this, task.getLocation().getWorld(),
                            task.getLocation().getChunk().getX(), task.getLocation().getChunk().getZ(), () -> {
        ...
        }, 1000L, readonlyConfig.blockProcessingTick * 50L, TimeUnit.MILLISECONDS);
